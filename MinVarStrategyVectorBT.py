...
MAX_WEIGHT    = 0.05   # 15 % cap per holding
...
def min_variance_weights(
    ...
    Returns a weight vector of length n, or equal-weight on failure.
    """
    w0 = np.full(n, 1.0 / n)
...
# ==============================================================================
# 3. COVARIANCE ESTIMATION  (replaces Signal Engine)
# ==============================================================================
...
for i, date in enumerate(rebal_dates):
    ...
    # Drop columns (assets) with any NaN in the window
    ...
    w_sub = min_variance_weights(
        ...
        sub_labels, list(set(sub_labels)), MAX_SECTOR,
    )
    ...
    if VOL_TARGET is not None:
        port_vol = portfolio_ex_ante_vol(w_full, np.diag(np.zeros(n_assets)))
        # Use the sub-universe cov for accurate vol estimate
        ...
        if port_vol > VOL_TARGET:
            ...
            w_full *= scale   # remainder (1-scale) stays in cash → vectorbt handles it
...
# Forward-fill between rebalance dates, fill first rows with equal weight
weights = weights.ffill()
first_valid = COV_LOOKBACK
eq_w = np.full(n_assets, 1.0 / n_assets)
weights.iloc[:first_valid] = weights.iloc[:first_valid].fillna(
    pd.Series(eq_w, index=prices.columns)
)
weights = weights.fillna(0)

# Final normalization (weights may not sum to 1 when vol-target scales them down)
...
vol_reduction = (bench_ann_vol - port_ann_vol) / bench_ann_vol   # % reduction
...
downside      = port_ret_real.clip(upper=0)
sortino       = port_ret_real.mean() / downside.std() * np.sqrt(252)
...
# ── Chart 4: Rolling realised volatility ─────────────────────────────────────  (NEW)
fig4 = go.Figure()
...
print("  → portfolio_volatility.html (rolling realised vol vs benchmark)")  # NEW